...
```
There are mainly 9 path modes. Examples for each can be found in `configs/paths_all_modes.ini`:
...
9) **hybrid spiral+raster** – square spiral (`spiral_dir = outward|inward`), source-off connector, then y-major raster.
...
-  `configs/laser_path.ini` &rarr; laser path definition (any of the 9 modes above).

Path configs now support lightweight inheritance:
...
The common DAG/run defaults live in [path_base.ini](/scratch/10226/shawnraul/Parallel_Hermes/configs/path_base.ini), while files such as [fast_heat.ini](/scratch/10226/shawnraul/Parallel_Hermes/configs/fast_heat.ini), [island_raster.ini](/scratch/10226/shawnraul/Parallel_Hermes/configs/island_raster.ini), [hilbert.ini](/scratch/10226/shawnraul/Parallel_Hermes/configs/hilbert.ini), and [hybrid_spiral_raster.ini](/scratch/10226/shawnraul/Parallel_Hermes/configs/hybrid_spiral_raster.ini) now only carry path geometry plus local overrides.

## Segment-Correction Planning

The segment-correction planner builds a supersegment DAG, extracts coupled and trivial regions, optionally refines only the safe trivial regions, and assigns the resulting runtime tasks to ranks for load balance.

Current planning model:

- `segments_per_supersegment` is the supersegment length used by the DAG pipeline
- runtime task cost is
  - phase 1: number of supersegments in the task
  - phase 2: `0.75` for each non-first-of-layer task
- `--split-mode off|greedy`
  - `off`: use DAG components directly
  - `greedy`: try safe cuts in trivial regions and accept only cuts that reduce predicted max rank load
...
python src/hermes/scripts/segment_correction/plan_only.py \
  ...
  --split-mode greedy \
  --apply-phase2 \
  --out-dir outputs/segment_plan
```

Automatic SS-length selection from atomic segmentation:
...
python src/hermes/scripts/segment_correction/plan_only.py \
  ...
  --split-mode greedy \
  --apply-phase2 \
  --auto-select-ss-length \
  --target-skew 1.10 \
  --out-dir outputs/plan_auto_ws4
```

The automatic search:

1. starts from `segments_per_supersegment = 1`
2. tests increasing SS lengths
3. early rejects candidates whose largest coupled block already implies a skew above the target
4. accepts the first candidate whose predicted skew is at most `target_skew`
5. otherwise falls back to the best candidate found

The planning summary is written to:

- `outputs/.../planning_summary.json`

For the full accepted design, see:

- `atomic_segment_dag_formulation.md`
