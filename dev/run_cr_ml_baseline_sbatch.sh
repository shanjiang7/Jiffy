#!/bin/bash
# Camera-ready experiment 2b, part 1: the 15-layer Bull 1-rank baseline
# (~3.5 h, single GPU, timing-only). On one rank the partitioner assigns the
# whole path either way, so the exact_dp baseline is copied as the uniform
# baseline instead of burning a second 3.5 h run (identical physics; the
# copy is logged).
#
# Usage:  sbatch dev/run_cr_ml_baseline_sbatch.sh
...
if [ ! -f "${OUT_DIR}/timing_summary.json" ]; then
  mkdir -p "${OUT_DIR}"
  echo "------ [$(date)] 15-layer bull: 1-rank baseline (timing-only) ------"
