...
`_build_fused_bridge_run` collapses those into a single run. This check asserts
...
def check(src: int, successors: list[int], n_components: int = 8, steps_each: int = 40) -> None:
    ...
    horizon = {cid: 2 for cid in ordered}
    common = dict(
        ...
        correction_horizon_ss_map=horizon,
    )

    per_successor = {
        dst: _build_bridge_run(src_component=src, dst_component=dst, **common)
