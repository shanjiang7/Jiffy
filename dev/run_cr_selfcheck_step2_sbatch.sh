#!/bin/bash
# Self-convergence ladder at the NEW default horizon step (+2 SS/iteration;
# --self-check-horizon-step deliberately NOT passed so the default is what
# gets tested). Bull + continuous-hybrid, tol1e4, 32 ranks / 31 cuts, 10
# iterations, stride-20 snapshots vs the stride-10 serial references.
# Prints the paper-style table (tab:self_convergence format):
#   iterate | true max rel-L2 | ||d_k||   + the cumulative-estimate check.
#
# Usage:  sbatch dev/run_cr_selfcheck_step2_sbatch.sh

#SBATCH -J cr_selfchk2
#SBATCH -N 8
#SBATCH -n 32
#SBATCH --ntasks-per-node=4
#SBATCH -t 4:00:00
#SBATCH -o logs/cr_selfchk2_%j.out
#SBATCH -e logs/cr_selfchk2_%j.err
#SBATCH -p gh
#SBATCH -A ASC21034
...
run_ladder () {
  ...
  if [ -f "${ROOT}/compare_${TAG}_iter${ITERS}/comparison_summary.json" ]; then
    echo " [$(date)] ${PATHKEY}: step-2 ladder already done, skipping"
    return
  fi

  echo "====== [$(date)] ${PATHKEY}/tol1e4: 32 ranks, ladder x${ITERS} @ default step (+2 SS) ======"
...
echo ""
echo "===== [$(date)] paper-style tables (step +2 SS/iter, 32 ranks, tol1e4) ====="
L=logs/cr_selfchk2_${SLURM_JOB_ID}.out
