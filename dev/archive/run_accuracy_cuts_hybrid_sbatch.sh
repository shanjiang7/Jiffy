#!/bin/bash
# Error-vs-cuts study (chords DAG) on the HYBRID spiral-raster path.
#
...
# Compared vs the serial references from job 838610. Completed comparisons are
# skipped, so the job is resumable if it times out.
#
# Companion script: run_accuracy_cuts_bull_sbatch.sh (longhorn path).
#
# Usage:  sbatch run_accuracy_cuts_hybrid_sbatch.sh

#SBATCH -J acc_cuts_hyb
#SBATCH -N 8
#SBATCH -n 32
#SBATCH --ntasks-per-node=4
#SBATCH -t 2:00:00
#SBATCH -o logs/acc_cuts_hyb_%j.out
#SBATCH -e logs/acc_cuts_hyb_%j.err
#SBATCH -p gh-dev
#SBATCH -A ASC21034
...
for TOL in tol1e4 tol1e7; do
  ...
  for SRC in chord src10; do
    ...
    echo "======================================================"
    echo " [$(date)] hybrid/${TOL}: parallel 32 ranks on 8 GPUs (chords, lookup=${SRC})"
    echo "======================================================"
    ...
    echo "------ [$(date)] hybrid/${TOL}/par32-${SRC}: rel-L2 comparison (source-on only) ------"
...
echo ""
echo "[$(date)] all done. Hybrid error vs number of partitions:"
for TOL in tol1e4 tol1e7; do
  for MODE in chords_chordsrc chords_src10 par32_chord par32_src10; do
    F=outputs/accuracy_hybrid_${TOL}_h30/compare_${MODE}/comparison_summary.json
    LABEL=${MODE/chords_chordsrc/par8  chord (7 cuts)}
...
print(f'  hybrid/${TOL} ${LABEL}: max={d[\"max_rel_l2\"]:.4e} mean={d[\"mean_rel_l2\"]:.4e} n={d[\"num_compared\"]}')"
  done
done
