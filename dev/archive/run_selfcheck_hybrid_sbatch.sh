#!/bin/bash
# Self-convergence ladder study (horizon-only mode) on the hybrid path:
...
#
# Usage:  sbatch dev/run_selfcheck_hybrid_sbatch.sh

#SBATCH -J selfchk_hybrid
#SBATCH -N 8
#SBATCH -n 32
#SBATCH --ntasks-per-node=4
#SBATCH -t 2:00:00
#SBATCH -o logs/selfchk_hybrid_%j.out
#SBATCH -e logs/selfchk_hybrid_%j.err
#SBATCH -p gh
#SBATCH -A ASC21034
...
echo "====== [$(date)] hybrid/tol1e4: 32 ranks, horizon-only self-check ladder x${ITERS} ======"
...
echo "------ [$(date)] hybrid: truth comparisons (production + each iterate) ------"
...
echo ""
echo "[$(date)] ===== hybrid: estimate vs true rel-L2 (31 cuts, tol1e4, horizon-only) ====="
L=$(ls -t logs/selfchk_hybrid_*.out | head -1)
...
def truth(suffix):
    try:
        d = json.load(open(f"outputs/accuracy_hybrid_tol1e4_h30/compare_par32_ladder{suffix}/comparison_summary.json"))
        return d["max_rel_l2"]
    except Exception:
        return None
