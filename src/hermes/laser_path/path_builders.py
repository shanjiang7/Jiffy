...
def build_hybrid_spiral_raster_sections_nd(
    *,
    origin_m: tuple[float, float],
    spiral_side_m: float,
    spiral_pitch_m: float,
    spiral_dir: str = "outward",
    ...
    repeats: int = 1,
    raster_dir_sign: int = +1,
    tile_connector_source_on: bool = False,
    len_scale: float,
) -> list[PathSectionND]:
    ...
    spiral_nd = build_square_spiral_nd(
        origin_m=origin_m,
        side_m=spiral_side_m,
        line_pitch_m=spiral_pitch_m,
        len_scale=len_scale,
        direction=spiral_dir,
    )

    x0_m, y0_m = float(origin_m[0]), float(origin_m[1])
    raster_origin_m = (x0_m + float(spiral_side_m) + float(gap_m), y0_m)
    raster_nd = build_raster_nd(
        origin_m=raster_origin_m,
        width_m=float(spiral_side_m),
        height_m=float(raster_width_m),
        line_pitch_m=float(raster_line_pitch_m),
        passes=None,
        x_dir_sign=int(raster_dir_sign),
        x_major=False,
        len_scale=len_scale,
    )

    connector_nd = np.vstack([spiral_nd[-1], raster_nd[0]])
    base_sections: list[PathSectionND] = [
        (spiral_nd, True),
        (connector_nd, False),
        (raster_nd, True),
    ]

    tile_span_m = float(spiral_side_m) + float(gap_m) + float(raster_width_m) + float(tile_gap_m)
    tile_span_nd = _to_nd(tile_span_m, len_scale)

    sections: list[PathSectionND] = []
    for tile_idx in range(int(repeats)):
        x_shift_nd = tile_idx * tile_span_nd
        shifted_tile: list[PathSectionND] = []
        for pts_nd, source_on in base_sections:
            shifted_tile.append((pts_nd + np.array([x_shift_nd, 0.0]), source_on))

        if sections:
            prev_last = sections[-1][0][-1]
            next_first = shifted_tile[0][0][0]
            sections.append((np.vstack([prev_last, next_first]), bool(tile_connector_source_on)))

        sections.extend(shifted_tile)
